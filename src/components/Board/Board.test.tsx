...
describe("Board の空状態", () => {
  it("タスクが1件も無いときは列を維持したまま Issue 一覧への誘導リンクを表示する", () => {
    ...
    // インラインの Issue 作成フォームは置かず、/issues への誘導リンクのみ
    // 出す（Issue #106・UI文言・配置規約.md §5）
  ...
  it("タスクがあるときは案内を出さず、カードを表示する", () => {
    ...
    expect(
      screen.queryByRole("link", { name: "Issue 一覧へ" }),
    ).not.toBeInTheDocument();
...
describe("Board の空状態メッセージの基準（Issue #92 再レビュー指摘3）", () => {
  it("ドラッグ中に live な columns が0件へ変化しても、表示中カードと矛盾する『Task がありません』は出さない", () => {
    boardQuery.mockReturnValue(createColumns({ todo: [createTask()] }));
