...
describe("Board のドラッグ直列化（Issue #92 4周目レビュー指摘1・2）", () => {
  ...
  it("mutation 未解決中に onDragStart が呼ばれても防御ガードにより無視され、board は不変・エラーも出ない", () => {
    const a = createTask({
      _id: "task_1" as Id<"tasks">,
      number: 1,
      rank: "a0",
    ...
    // その反映前のごく短い競合ウィンドウに対する防御として handleDragStart
    // 冒頭のガードが残っている。pending 中に onDragStart が呼ばれても
    // activeTask は設定されない（boardDragging クラスが付かないことで
    // 観測できる）。
    ...
    // activeTask が無いため onDragEnd を駆動しても no-op（エラーも出ない）。
    act(() => {
  ...
  it("pending 中に onDragCancel が呼ばれても resync せず、楽観状態を維持する（防御ガード）", () => {
    ...
    // により resync されない。
    const beforeCancel = dndHandlers.current;
    if (!beforeCancel) throw new Error("DndContext が描画されていません");
    act(() => {
