...
import { seedProject } from "../test/convexSupport";
...
describe("projects.create", () => {
  ...
  it("重複キーを拒否し、Project を追加しない（キー一意性の INVARIANT）", async () => {
    ...
    await expect(
      t.mutation(api.projects.create, { key: "TASK", name: "重複" }),
    ).rejects.toThrowError('プロジェクトキー "TASK" は既に使用されています');

    // 失敗したトランザクションは何も書き込まない
    expect(await t.query(api.projects.list, {})).toHaveLength(1);
  });
  ...
  it.each([
    ...
    await expect(
      t.mutation(api.projects.create, { key, name: "x" }),
  ...
  it.each([
    ...
    const id = await t.mutation(api.projects.create, { key, name: "x" });
...
describe("projects.getByKey", () => {
  it("キーに一致する Project を返す", async () => {
    ...
    const found = await t.query(api.projects.getByKey, { key: "TASK" });
  ...
  it("存在しないキーは null を返す", async () => {
    ...
    expect(await t.query(api.projects.getByKey, { key: "NONE" })).toBeNull();
  });
});
