...
| 削除・起動ボタン | 「削除」 | `IssueTable.tsx:177`「削除」（語は適合。配置は §5 で変更）／`TaskDetail.tsx:441`「タスクを削除」（語は適合だがエンティティ名が §2 に反する） |
| 削除・確認ボタン | 「削除する」 | `IssueTable.tsx:186` / `TaskDetail.tsx:242`（いずれも適合） |
...
すでに `none` の表示が食い違っている（TaskCard は「—」、`taskMeta.ts:16-18` は
「なし」）。重複排除の際は none の表示をどちらへ寄せるかを明示的に決めること
（挙動不変の機械的作業ではない）。
...
| 状態遷移 | Task 詳細のみ | 既存どおり（`TaskDetail.tsx:362-371`）。Issue のステータスは配下 Task から自動算出される派生値（詳細画面設計.md §5.1 `deriveIssueStatus`）のため、Issue 詳細には遷移ボタンを置かず「ステータスは配下 Task から自動算出されます」等の説明文を置く（現状 `IssueDetail.tsx:118` はバッジ表示のみで説明文が無い） |
...
- 適合例: `IssueTable.tsx:188`「この Issue と配下のタスク・Git 連携をすべて
  削除します。取り消せません。」
