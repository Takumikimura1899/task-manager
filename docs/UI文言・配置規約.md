...
| 削除・起動ボタン | 「Issue を削除」／「Task を削除」（詳細画面の danger セクションに置く。§5） | `TaskDetail.tsx:441`「タスクを削除」（エンティティ名が §2 に反する→「Task を削除」へ）。`IssueTable.tsx:177` の素の「削除」は #105 で導線ごと撤去するため統一対象外 |
...
| 状態遷移 | Task 詳細のみ | 既存どおり（`TaskDetail.tsx:362-371`）。Issue のステータスは配下 Task から自動算出される派生値（基本設計書.md §5.1「Issue の派生ステータス（ADR-10）」の `deriveIssueStatus`）のため、Issue 詳細には遷移ボタンを置かず「ステータスは配下 Task から自動算出されます」等の説明文を置く（現状 `IssueDetail.tsx:118` はバッジ表示のみで説明文が無い） |
